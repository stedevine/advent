...
def get_ingredients():
    ...
    for line in f:
        words = line.split(" ")
        ingredients.append({"name" : words[0].strip(":"), "cap" : int(words[2].strip(",")), "dur":int(words[4].strip(",")), "fla": int(words[6].strip(",")), "tex": int(words[8].strip(",")), "cal" : int(words[10])})
...
def get_properties(combination, ingredients):
    ...
    total_cal = (combination["i"] * ingredients[0]["cal"]) + (combination["j"] * ingredients[1]["cal"]) + (combination["k"] * ingredients[2]["cal"]) + (combination["l"] * ingredients[3]["cal"])
    if (total_cal != 500) :
        return 0
    #print(total_cap)
    #print(total_dur)
