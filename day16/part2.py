...
def remove_items_greater_than(item_name, count, sues):
    for sue in sues:
        not_sues = [sues.index(s) for s in sues if (item_name in s) and s[item_name] <= count]
...
def remove_items_fewer_than(item_name, count, sues):
    for sue in sues:
        not_sues = [sues.index(s) for s in sues if (item_name in s) and s[item_name] >= count]
...
remove_items_greater_than("cats", 7, sues)
remove_items("samoyeds", 2, sues)
remove_items_fewer_than("pomeranians", 3, sues)
remove_items("akitas", 0, sues)
remove_items("vizslas", 0, sues)
remove_items_fewer_than("goldfish", 5, sues)
remove_items_greater_than("trees", 3, sues)
